...
# shannon_index_less10 <- lapply(bact_fung_taxa_less10[-c(2, 5, 6, 8, 11, 12)], function(x) diversity(t(x), index = "shannon", base = exp(1)))
# hill_shannon_less10 <- lapply(bact_fung_taxa_less10[-c(2, 5, 6, 8, 11, 12)], function(x) hill_taxa(t(x), q = 1, MARGIN = 1, base = exp(1)))
#
# pielou_index_less10 <- lapply(bact_fung_taxa_less10[-c(2, 5, 6, 8, 11, 12)], function(x) diversity(t(x))/log(specnumber(t(x))))
#
# margalef_index_less10 <- lapply(bact_fung_taxa_less10[-c(2, 5, 6, 8, 11, 12)], function(x) apply(x, 2, margalef))
#
# simpson_index_less10 <- lapply(bact_fung_taxa_less10[-c(2, 5, 6, 8, 11, 12)], function(x) diversity(t(x), index = "simpson", MARGIN = 1, base = exp(1)))
```
...
#   repla <- gsub("_index_", "_", list_name) # Remove "_index_" from list_name
#   names(lista) <- lapply(names(lista), function(df) {
#     gsub("reads", repla, df) # Replace "reads" with the modified list_name
#   })
#   as.data.frame(lista)
# })
#
...
# env_data_normal <- as.data.frame(env_data_normal)
#
# indices2_less10 <- indices_less10[,c("Bacteria_species_shannon_less10", "Fungi_species_shannon_less10",
#           "Bacteria_genus_shannon_less10", "Fungi_genus_shannon_less10",
#           "Bacteria_family_shannon_less10", "Fungi_family_shannon_less10")]
#
...
fungi_div_500m <- nls(Fungi_species_diversidade ~ NLS.expoGrowth(LUI_500m, a, k), data = modelos)
...
```{r}
pdf("modelos_bacterias_fungos.pdf")
for (i in colnames(indices2)) {
  for(j in colnames(env_data)) {
    dados <- as.data.frame(cbind(indices2[[i]], env_data[[j]]))
    #print(dados)
    par(mfrow=c(1,1))
    plot(indices2[[i]] ~ env_data[[j]], data = dados,
         xlab = colnames(env_data[j]), ylab = colnames(indices2[i]))
  }
}
dev.off()
