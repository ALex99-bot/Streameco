...
```{r}
# Índices
diversidade <- lapply(bact_fung_taxa[-c(1, 5, 6, 7, 10, 11)], function(x) specnumber(t(x)))

shannon_index <- lapply(bact_fung_taxa[-c(1, 5, 6, 7, 10, 11)], function(x) diversity(t(x), index = "shannon", base = exp(1)))
hill_shannon <- lapply(bact_fung_taxa[-c(1, 5, 6, 7, 10, 11)], function(x) hill_taxa(t(x), q = 1, MARGIN = 1, base = exp(1)))

pielou_index <- lapply(bact_fung_taxa[-c(1, 5, 6, 7, 10, 11)], function(x) diversity(t(x))/log(specnumber(t(x))))

margalef_index <- lapply(bact_fung_taxa[-c(1, 5, 6, 7, 10, 11)], function(x) apply(x, 2, margalef))

simpson_index <- lapply(bact_fung_taxa[-c(1, 5, 6, 7, 10, 11)], function(x) diversity(t(x), index = "simpson", MARGIN = 1, base = exp(1)))

# Índices 10 menos representados
shannon_index_less10 <- lapply(bact_fung_taxa_less10[-c(1, 5, 6, 7, 10, 11)], function(x) diversity(t(x), index = "shannon", base = exp(1)))
hill_shannon_less10 <- lapply(bact_fung_taxa_less10[-c(1, 5, 6, 7, 10, 11)], function(x) hill_taxa(t(x), q = 1, MARGIN = 1, base = exp(1)))

pielou_index_less10 <- lapply(bact_fung_taxa_less10[-c(1, 5, 6, 7, 10, 11)], function(x) diversity(t(x))/log(specnumber(t(x))))

margalef_index_less10 <- lapply(bact_fung_taxa_less10[-c(1, 5, 6, 7, 10, 11)], function(x) apply(x, 2, margalef))

simpson_index_less10 <- lapply(bact_fung_taxa_less10[-c(1, 5, 6, 7, 10, 11)], function(x) diversity(t(x), index = "simpson", MARGIN = 1, base = exp(1)))
```



```{r}
env_data_hypho <- env_data[-40,]
...
# Menos 10 representados

all_lists_less10 <- list(
  # hill_shannon_less10 = hill_shannon,
  # pielou_index_less10 = pielou_index,
  # margalef_index_less10 = margalef_index,
  # simpson_index_less10 = simpson_index,
  shannon_index_less10 = shannon_index
)


indices_less10 <- lapply(names(all_lists_less10), function(list_name) {
  lista <- all_lists_less10[[list_name]]
  repla <- gsub("_?index$", "", list_name)
  names(lista) <- lapply(names(lista), function(df) gsub("reads", repla, df))
  as.data.frame(lista)
})

indices_less10 <- as.data.frame(do.call(cbind, indices_less10))

env_data_nt <- as.data.frame(env_data_nt)
env_data_normal <- as.data.frame(env_data_normal)

indices2_less10 <- indices_less10[,c("Fungi_species_diversidade_less10", "Fungi_species_shannon_less10")]

modelos_less10 <- as.data.frame(cbind(indices2_less10, env_data_normal))
modelos_less10 <- cbind(modelos_less10, vel = modelos_less10$mean.Velocity)

modelos_nt_less10 <- as.data.frame(cbind(indices2_less10, env_data_nt))
modelos_nt_less10 <- cbind(modelos_nt_less10, vel = modelos_nt_less10$mean.Velocity)
...
```{r}
### Hifomicetos ###
hypho <- lapply(read_excel_sheets(hypho_path, hypho_file), excel2dataframe)[[1]]

PCA_env_hypho <- dudi.pca(env_data_hypho, center = TRUE, scale = TRUE, nf=5, scannf = FALSE)
biplot(PCA_env_hypho)
s.arrow(PCA_env_hypho$c1, lab=names(PCA_env_hypho$tab))
var_exp <- (PCA_env_hypho$eig*100)/sum(PCA_env_hypho$eig)

env_data_hypho <- cbind(env_data_hypho, PC1 = PCA_env_hypho$li[,1])

# índices
diversidade_hypho <- specnumber(t(hypho))
shannon_index_hypho <- diversity(t(hypho), index = "shannon")

ind_hypho <- as.data.frame(cbind(diversidade_hypho, shannon_index_hypho))

# Criar dataframe com dados todos juntos
# env_data_hypho <- as.data.frame(env_data_hypho)
modelos_hypho <- as.data.frame(cbind(data.frame(diversidade = diversidade_hypho), data.frame(shannon_index = shannon_index_hypho), env_data_hypho))
modelos_hypho <- cbind(modelos_hypho, vel = modelos_hypho$mean.Velocity)
...
# div ~ PC1
plot(diversidade ~ PC1, data = modelos_hypho)

# shannon ~ PC1
plot(shannon_index ~ PC1, data = modelos_hypho)

# div ~ qbr
div_qbr <- nls(diversidade ~ NLS.expoDecay(qbr, a, k), data = modelos_hypho)

# _div ~ alt
div_alt <- nls(diversidade ~ SSgauss(Altitude, mu, sigma, h), data = modelos_hypho)

# Bacteria_species_div ~ LUI_500m
div_500m <- nls(diversidade ~ NLS.expoGrowth(LUI_500m, a, k), data = modelos_hypho)

# div ~ DOmin
div_DOmin <- nls(diversidade ~ NLS.expoGrowth(DOmin, a, k), data = modelos_hypho)

# div ~ cond não segue uma distribuição gaussiana
# bact_div_cond <- nls(Bacteria_species_diversidade ~ SSgauss(cond, mu, sigma, h), data = modelos_hypho)

# _div ~ DIN
div_din <- nls(diversidade ~ SSgauss(DIN, mu, sigma, h), data = modelos_hypho)

# div ~ P.PO4
# modelos_retirado <- modelos_hypho[!(row.names(modelos_hypho) %in% c("LAB1", "NES1", "SAL1")),]
# ggplot(modelos_retirado, aes(P.PO4, Bacteria_species_diversidade)) +
#   geom_point() +
#   geom_text_repel(aes(label = rownames(modelos_retirado)))

div_po4 <- nls(Bacteria_species_diversidade ~ SSgauss(P.PO4, mu, sigma, h), data = modelos_hypho)

# Bdiv ~ Tmax
# bact_div_tmax <- nls(Bacteria_species_diversidade ~ NLS.bragg.3(Tmax, b, d, e), data = modelos_hypho)
div_tmax <- nls(Bacteria_species_diversidade ~ SSgauss(Tmax, mu, sigma, h), data = modelos_hypho)


# shannon ~ Discharge
shannon_discharge <- nls(Bacteria_species_shannon ~ NLS.expoDecay(Discharge, a, k), data = modelos_hypho)

# shannon ~ Tmax
shannon_tmax <- nls(Bacteria_species_shannon ~ NLS.expoDecay(Tmax, a, k), data = modelos_hypho)

# shannon ~ subasin
shannon_subasin <- nls(Bacteria_species_shannon ~ SSgauss(LUI_subasin, mu, sigma, h), data = modelos_hypho)

# shannon ~ mean.Velocity
# modelos_retirado <- modelos_hypho[!(row.names(modelos_hypho) %in% c("CBR1", "TAve2")),]

bact_shannon_mv <- nls(Bacteria_species_shannon ~ SSgauss(mean.Velocity, mu, sigma, h), data = modelos_hypho)
...
```{r}
pdf("modelos_hifomicetos.pdf")
for (i in colnames(ind_hypho)) {
  for(j in colnames(env_data_hypho)) {
    dados <- as.data.frame(cbind(ind_hypho[[i]], env_data_hypho[[j]]))
    #print(dados)
    par(mfrow=c(1,1))
    plot(ind_hypho[[i]] ~ env_data_hypho[[j]], data = dados,
         xlab = colnames(env_data_hypho[j]), ylab = colnames(ind_hypho[i]))
  }
}
dev.off()
```
```{r}
pdf("modelos_menos10.pdf")
for (i in colnames(indices_less10)) {
  for(j in colnames(env_data)) {
    dados <- as.data.frame(cbind(indices_less10[[i]], env_data[[j]]))
    #print(dados)
    par(mfrow=c(1,1))
    plot(indices_less10[[i]] ~ env_data[[j]], data = dados,
         xlab = colnames(env_data[j]), ylab = colnames(indices_less10[i]))
  }
}
dev.off()
